...
const options = {
  ...
  preventAttackerShieldBrake: true, // Prevent attack if after attack attacker shield will be broken
  protected: [] as string[], // List of protected fleet public keys
};
/**
...
async function run() {
  ...
  const battleLocation = new Coordinates(0, 61);
  ...
  while (true) {
    ...
    if (fa.state.MoveWarp) {
      await new ExitWarpAction(proc).run();
    } else if (fa.state.MoveSubwarp) {
      await new ExitSubwarpAction(proc).run();
    }
    while (1) {
      console.error(" ----------- ");
      fa = await proc.fetchFleetAccount();

      let state = await proc.dispatcher.sageGameHandler.sageFleetHandler.canAttack(fa, {
        funder: proc.dispatcher.signer.as,
        funderPermissionIdex: 0,
      });

      log("Can attack state", { attacks: state.attacksAvailable, canAttack: state.canAttack, reason: state.sateLabels });
      if (state.attacksAvailable < 2) break;
      /**
       * ! FIND TARGETS
       */
      ...
      while (tgtAccount) {
        ...
        if (options.preventSuicide && !(await preventSuicide(proc.fleetAccount as any, tgtAccount))) continue;
        if (options.preventAttackerShieldBrake && !(await preventAttackerShieldBrake(proc.fleetAccount as any, tgtAccount))) continue;
        break;
      }
      ...
      log(await dispatcher.sageFleetHandler.canAttack(fa));
       ...
       */
      if (tgtAccount) {
        let noAmoState = state.attacksAvailable < 3;
        console.error("Attacking target:", tgtAccount.key.toBase58());
        ...
        // !!!!!! No need to Attack
        // await action.run();
        await action.waitTimeProgress(10000, "WAITING TO BE ATTACKED", 1000);
...
async function load(proc: Process, fuelCost: number) {
  let fa = await proc.fetchFleetAccount();
  if (!fa) throw new Error("No fleet account found");
  if (fa.state.MoveWarp) {
    await new ExitWarpAction(proc).run();
    await new DockAction(proc).run();
  } else if (fa.state.MoveSubwarp) {
    await new ExitSubwarpAction(proc).run();
    await new DockAction(proc).run();
  }
