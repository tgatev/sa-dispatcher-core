...
async function run() {
  ...
  sduProcessor.surveyDataUnitTracker = new PublicKey("DXPsKQPMyaDtunxDWqiKTGWbQga3Wihck8zb8iSLATJQ"); //
  sduProcessor.before = async (logs: Logs) => {
    // console.log("SDU Logs:", logs);
    return logs.logs;
  };
  // dispatcher.sageFleetHandler.getFleetAccount()
  /** Provide pointer reference to handlers */
  let minChance = 0.1;
  let targetSector = new Coordinates(15, 30);
  let saveBase = new Coordinates(40, 30); // MRZ-23
  let fleetsScanArray = new Map<string, iScanArrayElement>();
  for (let i = 1; i <= 2; i++) {
    if (i === 13) continue;
    console.log("Fleet ", "scan" + (i || ""));
    // fleetsScanArray.set("Leopard Fleet", {
    fleetsScanArray.set("scan" + (i || ""), {
      fleetName: "scan" + (i || ""),
      // fleetName: "Leopard Fleet",
      ...
      scanConfig: {
        ...
        targetSector: new Coordinates(targetSector.x + i, targetSector.y - i), // ++--i
  ...
  scanProcess.doNotSimulateScans = true; // !! HOLOSIM SETUP - skip simulation step for scanning transactions - just send with current priority fee config - because in holosim we dont have real mempool and simulation is not needed ALSO when have no sector accounts exiting simulations always fails

  scanProcess.scaleFactor = 1000; // !! HOLOSIM SETUP
  // await new Promise((resolve) => setTimeout(resolve, 1 * 60 * 1000));

  scanProcess._sectorDefaultChance = minChance + 0.01; // predicted SDU chance for sectors without data from SDU Parser
