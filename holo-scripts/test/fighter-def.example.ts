...
async function run() {
  ...
  // let sectors = { home: { x: 30, y: 28 }, battle: { x: 17, y: 21 } };
  await Promise.all([
...
async function allFighters(
  ...
  opsFighter.withRepairStarbase = true;
  ...
  opsFighter.ratingFormula = (input, w) => {
    const dpsProxy = Math.max(0, input.ap) / (1 + Math.max(0, input.apCooldownSec)); // AP с penalty за cooldown
