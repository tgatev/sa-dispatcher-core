...
async function run() {
  ...
  let resAll = await dispatcher.sageGameHandler.profileFactionProgram.account.profileFactionAccount.all();
  // let factions = new Set(resAll.map((r) => r.account.faction));
  let sbData = await dispatcher.sageGameHandler.getStarbaseAccount(new PublicKey("8FpeDoiefTc53rM7xygV9r1XEZegNUbYub1ewRDY4Fdo"));
  log(sbData);
  // log(resAll);
  throw "DDDDD";
