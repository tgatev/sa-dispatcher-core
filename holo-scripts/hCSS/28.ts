import { buildLootRetrieveHealerActions, MiningBuildOptions } from "../../src/Model/FleetProcess";
...
async function run() {
  const saveStarbaseName = "mrz28"; // ust1 ust2 ust3 ust4 ust5 - mrz21
  ...
  proc.addAction(new CustomCombatLogAction(proc, { recommendRepair: false, waitShieldUp: true }));
  await buildScenario(
    proc,
    Resource.carbon,
    "mrz28",
    {
      miningTimes: 1,
      bundleDockUndock: true, // add undock and dock for each mining loop to test bundle handling
    },
    1,
  );
  proc.addAction(new CustomCombatLogAction(proc, { recommendRepair: false, waitShieldUp: true }));
