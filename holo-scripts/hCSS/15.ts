...
async function run() {
  const saveStarbaseName = "mrz23"; // ust1 ust2 ust3 ust4 ust5 - mrz21
  ...
  // proc.addAction(new CustomCombatLogAction(proc, { recommendRepair: false, waitApReload: true, waitShieldUp: true })); // Add custom action to log combat data and wait for shield regen if needed after mining action);
  // await buildScenario(
  //   proc,
  //   Resource.hydrogen,
  //   "mrz23",
  ...
  // proc.addAction(new CustomCombatLogAction(proc, true, false)); // Add custom action to log combat data and wait for shield regen if needed after mining action);
