...
async function run() {
  const saveStarbaseName = "mrz22"; // ust1 ust2 ust3 ust4 ust5 - mrz21
  ...
  proc.addAction(new CustomCombatLogAction(proc, { recommendRepair: true , waitShieldUp: true, waitApReload: true })); // Add custom action to log combat data and wait for shield regen if needed after mining action);
  // await buildScenario(
  //   proc,
  //   Resource.copper_ore,
  //   "mrz22",
  ...
  // proc.addAction(new CustomCombatLogAction(proc, true, true)); // Add custom action to log combat data and wait for shield regen if needed after mining action);
