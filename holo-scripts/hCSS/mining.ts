...
async function run() {
  ...
  let ammoCost = (await startMiningAction.getResourceCost()).ammunitions || 0;
  let fuelCost = (await startMiningAction.getResourceCost()).fuel || 0;
  ...
  let fleetReload = new TransferCargoAction(proc, [
    ...
    },
    {
      ...
      condition: { whenLessThen: ammoCost + 1 },
    },
  ]);
