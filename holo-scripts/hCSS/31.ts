...
async function run() {
  const saveStarbaseName = "mrz31"; // ust1 ust2 ust3 ust4 ust5 - mrz21
  ...
  let fuelLoad = (proc.actionsChain[1] as TransferCargoAction).resources.find(
  ...
  // (proc.actionsChain[2] as StartMiningAction).options.miningTime = 210; // s
...
async function buildScenario(proc: Process, resourceName: string, miningBase: string, options: MiningBuildOptions, times = 1) {
  ...
  proc.actionsChain.push(new CustomCombatLogAction(proc, true, true)); // Add custom action to log combat data and wait for shield regen if needed after mining action);
