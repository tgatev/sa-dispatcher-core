 ...
 */
export class FleetStateHandlerAction extends Action implements iSimpleAction {
  ...
  async execute(): Promise<Array<DispatcherParsedTransactionWithMeta | SimulatedTransactionResponse | undefined>> {
    // Fleet is Idle ( cant undock )
    // if ((await this.process.fetchFleetAccount()).state.Idle) {
    //   throw `Fleet is Idle - can't undock  + [${this.process.fleetName}]`;
    // }
  ...
  async getInstructionsReturns(): Promise<InstructionReturn[]> {
    return await this.dispatcher.sageFleetHandler.ixFleetStateStarbaseHandler(
  ...
  accumulateFees() {
    this.results.execution.forEach((trx) => {
      ...
      FleetStateHandlerAction.accumulatedTransactionCost += this.results.transactionFees || 0;
    });

    FleetStateHandlerAction.accumulatedRunTime += this.results.runTime || 0;
    super.accumulateFees();
  }

  async display(verbose = false): Promise<string> {
    let display =
      `FleetStateHandlerAction: T<${this.getTransactionCount()}> ` + (this.sector ? `sector: ${this.sector.toSectorKey()} ` : "");
  ...
  getTransactionCount() {
    return FleetStateHandlerAction.transactionsCount;
  }
