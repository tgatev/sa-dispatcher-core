...
import { gameDataEquals, SagePlayerProfile, StarbasePlayerAccount } from "@staratlas/sage-main";
...
import { CraftingInstance, CraftingInstanceType } from "./mod/craftingInstance";
...
export class StarbaseHandler {
  ...
  constructor(private _gameHandler: SageGameHandler) {
    ...
    Object.keys(SageGameHandler.starbaseMap).forEach((baseKey) => {
      try {
        this.starbaseKeys[baseKey] = new PublicKey(this._gameHandler.asStatic().starbaseMap[baseKey].starbasePublicKey!);
      } catch (e) {
  ...
  async init(gameHandler: SageGameHandler) {
    ...
    // if (!Array.isArray(StarbaseHandler.rawRecipeCategories) || StarbaseHandler.rawRecipes.length == 0)
    //   promises.push(StarbaseHandler.fetchCraftRecipesCategories(this.craftingProgram));
  ...
  /**
   * Provide Starbase Player Accounts by Player Profile
   *  - 1 per starbase
   * @param playerProfileKey
   * @returns
   */
  ...
  async getStarbaseEscrow(playerProfile: PublicKey, starbaseKey: PublicKey) {
    let playerData = await this._gameHandler.getStarbasePlayerAccount(playerProfile, starbaseKey);
    return playerData.wrappedShipEscrows;
  }
  /**
  ...
  /**
   *  Get Starbase Player Inventory by Cargo Pod
   * @param cargo
   * @returns
   */
 ...
 */
  ...
  async getStarbaseAccounts() {
    ...
    if ((starbases[0].type = "ok")) {
      // let {} = starbases[0].data as DecodedAccountData<Starbase>;
    }
  }
  /**
  ...
  > {
    ...
    if (filter) {
      return StarbaseHandler.rawRecipeCategories.filter((item) => {
        // @ts-ignore console.log("item.account.namespace", item.account.namespace);
...
function getTargetRecipe(starbasePlayerCargoHoldsAndTokens: any, userCraft: any, targetAmount: number) {
