...
const RESOURCE = RESOURCE_COPPER_OER;
const MINING_SECONDS_MRZ28 = 17 * 60 + 32;
const MINING_SECONDS_USTUR_CSS = 26 * 60 + 22;
const MINING_SECONDS_MRZ_35 = 35 * 60 + 25;
const MINING_SECONDS = MINING_SECONDS_MRZ_35;
const ITERATIONS = 10;
...
const run = async () => {
  ...
  //@ts-ignore
  const fleetAmmoCapacity = fleetAccount.data.stats.cargoStats.ammoCapacity;
  console.log("Ammo capacity: ", fleetAmmoCapacity);
  //@ts-ignore
  const fleetFuelCapacity = fleetAccount.data.stats.cargoStats.fuelCapacity;
  ...
  if (fleetAccount.state.StarbaseLoadingBay?.starbase && !fleetAccount.state.Idle) {
    ...
    console.log(`Fuel after loading: `, fuelAfterLoad, ` Load Food:  `, MINING_COST.ammunitions);
