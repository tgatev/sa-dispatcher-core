...
export async function run(repeat: number | undefined = undefined) {
  ...
  let miningResourceUST23 = await SageGameHandler.readStarbaseResource(miningBaseUST23, "biomass");
  ...
  let optionsUST23 = {
    ...
    pathToMiningStarbase: [new Coordinates(43, 10), miningBaseUST23.location],
    pathToSafeStarbase: [new Coordinates(36, 16), proc.saveStarbase],
    transportToMiningBase: [{ resourceName: "copper_ore", percent: 1 }],
    transportToSafeStarbase: [
      // { resourceName: "iron_ore", percent: 1 },
