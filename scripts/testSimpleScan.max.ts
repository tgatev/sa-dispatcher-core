...
async function run() {
  ...
  fleetsScanArray.set("s1", {
    ...
    scanConfig: {
      ...
      targetAreaSize: 12 + 5 + 3,
      // Form of area
      targetAreaType: "Square",
      // Center point of area
      targetSector: new Coordinates(3, -1),
  ...
  scanProcess._sectorDefaultChance = minChance + 0.001; // predicted SDU chance for sectors whithout data from SDU Parser
  ...
  scanProcess._sectorDataFreshPeriod = 40 * 60; // 5 minutes - SDU Processor - do not use old data as SDU Chance for flat value
  ...
  scanProcess._sectorDisableTime = 30 * 60;
  scanProcess._waitBeforeMove = 15 * 60; // 4 minutes - after cooldown expire waite sector chance to rise up
