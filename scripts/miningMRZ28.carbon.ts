...
const RESOURCE = RESOURCE_CARBON;
...
const run = async () => {
  ...
  async function signAndSend(ixf: CallableFunction, skipOnError = false) {
    let ix = await ixf();
    let tx, rx;
    console.log("Ix.length: ", ix.length);
  ...
  //@ts-ignore
  const fleetAmmoCapacity = fleetAccount.data.stats.cargoStats.ammoCapacity.toString();
  console.log("Ammo capacity: ", fleetAmmoCapacity);
  //@ts-ignore
  const fleetFuelCapacity = fleetAccount.data.stats.cargoStats.fuelCapacity.toString();
  ...
  rx = await signAndSend(async () => await sageFleetHandler.ixStopMining(fleetPubkey));
  ...
  rx = await signAndSend(async () => await sageFleetHandler.ixDockToStarbase(fleetPubkey));
  ...
  rx = await signAndSend(async () => await sageFleetHandler.ixWithdrawCargoFromFleet(fleetPubkey, resourceToken, new BN(9_999_999)));
