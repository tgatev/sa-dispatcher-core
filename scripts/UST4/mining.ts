...
/**
 * Provide base mining scenario on ustur 4
 *  process.build example and base Actions following
 */
let fleetName = argv.fleetName;
let resource = argv.resource;
const miningTime = argv.miningTime || null;

async function run() {
  /** Provide pointer reference to handlers */
  let proc = await Process.build(fleetName, "ust4");
  let starbase = SageGameHandler.starbaseMap.ust4;
  resource = SageGameHandler.readStarbaseResource(starbase, resource);
  let hardness = SageGameHandler.resourceHardness[resource];

  let startMiningAction = new StartMiningAction(
    proc,
    resource, // resource
    hardness, // hardness - should getfrom mine account that will be minned
    starbase.richness, // richness
